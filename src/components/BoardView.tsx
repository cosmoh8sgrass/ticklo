...
const BoardView: React.FC = () => {
  const { groups, selectedGroupId, addTask, updateTask, moveTask } = useStore();
  ...
  const TaskCard: React.FC<{ task: Task; status: string }> = ({ task, status }) => (
    <motion.div
      ...
      className={`p-3 bg-white rounded-lg border-l-4 shadow-sm hover:shadow-md transition-all cursor-move ${getPriorityColor(task.priority)}`}
    >
  ...
  const Column: React.FC<{
    ...
    <div className="flex-1 min-w-0">
      ...
      <div
        className="min-h-[400px] p-3 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200"
